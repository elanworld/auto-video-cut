...
public class BulletApply {
    ...
    public List<String> getFiles() {
        ...
        for (String dir : this.videos) {
            allFile.addAll(FilesBox.dictoryList(dir));
        }
        return allFile;
    }
    ...
    public void run() {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            fFmpegCmd.setInput(episodeFile).setOutput(FilesBox.outDirFile(episodeFile))
                    .setTime(start,end).setCodecQSV().run();
        }
    ...
    public void addVideoDir(String videoDir) {
        this.videos.addAll(FilesBox.dictoryList(videoDir));
    }
